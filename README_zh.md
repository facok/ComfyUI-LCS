...
LCS 在扩散采样**过程中**操作，而非生成后——这是与传统调色的核心区别。

| | 后处理 | LCS |
|---|---|---|
| **时机** | VAE 解码后，在像素空间 | 采样过程中，在潜在空间 |
| **机制** | 对最终图像施加颜色滤镜 | 在生成中途修改 3D 颜色子空间 |
| **模型感知** | 无——结构已经定型 | 模型在后续步骤中自适应颜色偏移 |
| **效果** | 颜色可能看起来像"涂上去的"——阴影/肤色容易不自然 | 颜色像模型本来就打算这样生成——内容自然协调 |

例子：想要暖橙色调的日落，后处理会给整张图叠橙色（阴影变脏），而 LCS 在采样早期推动颜色，模型生成的云层、光照、反射都与暖色调**内在一致**。

论文核心发现：颜色和结构在潜在 patch 空间中是**正交**的，可以单独控制颜色而不干扰结构——在像素空间中无法做到，因为颜色和结构是纠缠在一起的。
